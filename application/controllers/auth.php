...
class Auth extends CI_Controller {
    ...
    function index() {
        ...
        if (!$this->ion_auth->logged_in()) {
            ...
            //$this->data['nombres'] = mb_convert_case($this->regconductor_model->get_nombre($this->ion_auth->user()->row()->username)->nombres, MB_CASE_TITLE, "UTF-8");
    ...
    function create_user() {
        ...
        $this->form_validation->set_rules('nombre_completo', 'Nombre Completo');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $additional_data = array(
                'nombres' => $this->input->post('nombre_completo')
            );
        }
        if ($this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email, $additional_data)) {
            ...
            $this->data['nombre'] = mb_convert_case($this->ion_auth->user()->row()->nombres, MB_CASE_TITLE, "UTF-8");
    ...
    function edit_user($id) {
        ...
        //validate form input
        $this->form_validation->set_rules('nombre_completo', $this->lang->line('edit_user_validation_fname_label'), 'required');
...
//                'phone' => $this->input->post('phone'),
        ...
        $this->data['nombre_completo'] = array(
            ...
            'id' => 'nombre_completo',
            'type' => 'text',
            'value' => $this->form_validation->set_value('nombre_completo', $user->nombres),
                //'value' => 'Carlos',
        );
        ...
        $this->data['nombre'] = mb_convert_case($this->ion_auth->user()->row()->nombres, MB_CASE_TITLE, "UTF-8");
        $this->data['template'] = 'auth/edit_user';
    ...
    function actualizacion_users_usuarios() {
        if (!$this->ion_auth->logged_in()) {
            //redirect them to the login page
            redirect('auth/login', 'refresh');
        } elseif (!$this->ion_auth->is_admin()) { //remove this elseif if you want to enable this for non-admins
            //redirect them to the home page because they must be an administrator to view this
            return show_error('Solo los Admins pueden ejecutar esta accion');
        } else {
            $res = array();
            $this->load->model('mireserva_model');
            $usuarios_activos = $this->mireserva_model->traer_usuarios();
            $actualizados = 0;
            
            foreach ($usuarios_activos as $user) {
                
                $group = array($user->tipo_usuario=>$user->tipo_usuario);
                
                               //print_r($group); 
                               //die;
                $additional_data = array('nombres' => mb_convert_case($user->nombre, MB_CASE_TITLE, "UTF-8"));
                //,'ultimo_cambio_clave'=>date("Y-m-d H:i:s")
                $r = $this->ion_auth->register($user->cedula, $user->cedula, $user->correo, $additional_data,$group);
